...
Four phase points at $\theta=0,\pi/2,\pi,3\pi/2$ on $\Orb$,
cycled by $\phi^{\pi/2}$:
$p_I=(1,0)$, $p_{VI}=(1,\pi/2)$, $p_{III}=(1,\pi)$,
$p_{VII}=(1,3\pi/2)$.

\textbf{Axiom~2 (Generative closure).}
$\phi^t(\Orb)=\Orb$ for all $t$: on $\Orb$, $r=1$ and
$\dot r=1\cdot(1-1)=0$, so $r$ remains $1$.

\textbf{Axiom~3 (Structural primacy).}
$V=(r-1)^2$, $\dot V=2(r-1)\dot r=2(r-1)r(1-r^2)
=-2(r-1)^2 r(1+r)<0$ for $r\ne 1$.

\textbf{Axiom~4 (Perturbation response).}
Near $r=1$: $\dot V\approx-4(r-1)^2=-4V$, so $c=4$.

\textbf{Axiom~5 (Noise-as-fuel).}
Generator: $\cL V=\dot V+\frac{\sigma_0^2}{2}\cdot 2=-4V+\sigma_0^2$,
so $\kappa=1$ and the condition holds with $c=4$, $\kappa=1$.

\textbf{Axiom~6 (Non-collapse).}
$\dot\theta=1\ne 0$ on $\Orb$, $T^*=2\pi>0$.

\textbf{Axiom~7 (Non-coercion).}
$Y$ is $C^\infty$ on $S$; no hard constraints.
