...
The contact dissipation
$H_{\mathrm{diss}}(x,z) = -\gamma V(x)e^{-\beta z}$
is a smooth regularization of $S(\gamma) =
\mu\,\Theta(\kappa(\gamma)-\kappa^*)$: as $\beta\to\infty$ and
$z\to 0^+$, the correction $-\gamma\nabla V e^{-\beta z}$
concentrates near $\Orb=\{V=0\}$ and mimics the threshold
activation of $\Theta$ at $\kappa=\kappa^*$.
\end{proposition}

\begin{proof}[Proof sketch]
For large $\beta$, $e^{-\beta z}\to 0$ away from $z\approx 0$,
so $H_{\mathrm{diss}}$ is negligible in the post-embodiment
regime. Near $z=0$, $V\approx 0$, the correction provides an
impulsive push toward $\Orb$ of magnitude
$\mu=\gamma\norm{\nabla V}$. In the limit $\beta\to\infty$,
this recovers step-function activation, corresponding to $S$.
\end{proof}
